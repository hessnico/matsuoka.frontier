...
description: >
  Learn how to use the MatsuokaDistribution class to estimate the
  production frontier using the three-step approach.
vignette: >
  %\VignetteIndexEntry{matsuoka3step}
...
4. Registering a new method with `register_strategy()` and using it
5. Without using the wrapper `matsuoka3step`
6. Cramer-Von Mises test with the Matsuoka's distribution after estimating Matsuoka parameter
...
The package does not have an automated plot function to extract `g(x)` components.
However, since the `matsuoka3step` saves the `g(x)` estimated object, it is possible to extract the components through it, and diagnose each case, in our example case, we can:
...
plot(x1, y, ylim = c(4,10), type = "n")
lines(sort(x1), comp_1[order(x1)], col = "blue", lwd = 2)
plot(x2, y, ylim = c(4,10), type = "n")
...
There is a built-in plot function that can be used, the first graph is related to the Matsuoka density:
```{r gam mat plot 1}
plot(res, which = 1)
```

And the second one, is a contour plot, for multivariate dataframes:
```{r gam mat plot 2}
plot(res, which = 2)
```

The built-in plot function has optional args `...`, with that is possible to enhance the contour plot:
```{r}
plot(res, which = 2, ngrid = 200, counter_levels = 15)
```
...
The `matsuoka3step` estimation function inherits properties from S3 objects:
```{r specific}
...
## Registering a new method with `register_strategy()` and using it

With `register_strategy`, it is possible to create a custom strategy, register it and extend the `g(x)` functions to be used, as shown below:
```{r new strategy}
custom_knn_strategy_example <- function(X, z, k, ...) {
    ...
    return(list(
        estimate = g_hat,
        model = list(k = k, distance = "euclidean"),
        meta = list(
            method = "my_custom_knn_local_strategy",
            call = match.call()
        )
    ))
}
...
g = "knn_local"
matsuoka.frontier::register_strategy(g, custom_knn_strategy_example)

res.knn <- matsuoka.frontier::matsuoka3step(
    x = x.df,
    y = y,
    g = "knn_local",
    k = 20
)
summary(res.knn$g_hat)
summary(res.knn)
plot(res.knn)
```
...
To a better diagnose of the Matsuoka’s distribution, it was also implemented the Cramer-von Mises test to verify adherence of the estimated efficiency:

```{r}
res <- matsuoka.frontier::matsuoka3step(x = x.df, y = y, 
                                        g = "backf.cl",
                                        windows = c(1,1))

ef <- res$y / res$f_hat
cvm_matsuoka_test(ef, p = res$p_hat)
```

And finally, a visual plot to compare:
```{r}
visual_cvm_matsuoka(ef, p = res$p_hat, nullname = "test")
```
